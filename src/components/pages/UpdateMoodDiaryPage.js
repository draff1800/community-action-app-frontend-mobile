...
export default class UpdateMoodDiaryPage extends Component{
  constructor(props) {
    super(props);
    this.state = {
      selectedDate: this.props.route.params.diaryDate,
      selectedTime: this.props.route.params.diaryTime,
      selectedMood: this.props.route.params.diaryMood,
      selectedCategory: this.props.route.params.diaryCategory,
      description: this.props.route.params.diaryDescription
    };
  }

  handleSave = () => {
    if(this.state.selectedTime != '' && this.state.selectedMood !== '' && this.state.selectedCategory != '' && this.state.description != '') {
      fetch('http://192.168.0.18:8080/updateMoodDiary/' + this.props.route.params.diaryID, {
        method: 'PUT',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({diaryDate:this.state.selectedDate,
        ...
        if(response.status === 200) {
          ToastAndroid.show('Diary updated!', ToastAndroid.SHORT);
          this.props.navigation.navigate('Mood Diaries')
        } else {
          console.log("HHHHHHEY", this.props.route.params);
                ...
                <RadioForm formHorizontal={true} animation={true}>
                  <RadioButton labelHorizontal={true} key={0} >
                    <RadioButtonInput
                      obj={{label: '', value: 0}}
                      index={0}
                      isSelected={this.state.selectedMood == 0}
